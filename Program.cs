...
var controller = new FlashcardController(serviceProvider);
...
static async Task CustomSeeding()
{
    ...
    var stack = await context.Stacks.FirstOrDefaultAsync();
    ...
    await context.Stacks.AddRangeAsync(stacks);
    await context.SaveChangesAsync();

    var card1 = new Flashcard { Question = "What is natural selection?", Answer = "The process that results in the continued existence of only the types of animals and plants that are best able to produce young or new plants in the conditions in which they live.", StackId = biologyStack.Id };
    var card2 = new Flashcard { Question = "What is evolution?", Answer = "The result of natural selection.", StackId = biologyStack.Id };
    var card3 = new Flashcard { Question = "What do DNA stands for?", Answer = "Deoxyribonucleic acid.", StackId = biologyStack.Id };
    List<Flashcard> cards = [card1, card2, card3];

    var card4 = new Flashcard { Question = "What is a stack?", Answer = "A stack is a linear data structure following the LIFO (Last-In, First-Out) principle.", StackId = csStack.Id };
    var card5 = new Flashcard { Question = "What is the ALU?", Answer = "Arithmetic-logic unit, the component of a processor responsible for doing mathematical and logical operations.", StackId = csStack.Id };
    var card6 = new Flashcard { Question = "Briefly explain the big O notation.", Answer = "Big O notation is used to classify algorithms according to how their run time or space requirements grow as the input size grows.", StackId = csStack.Id };
    cards.AddRange(card4, card5, card6);

    var card7 = new Flashcard { Question = "List the 2 postulates of special relativity.", Answer = "1. The laws of physics are identical in all inertial frames\n2. The speed of light in a vacuum is constant in all inertial frames.", StackId = physicsStack.Id };
    var card8 = new Flashcard { Question = "List the 3 laws of Newton.", Answer = "1. Inertia: an object at rest or moving at constant speed retains that speed until acted upon by a force.\n2. A force causes an accelaration according to the formula: F=m.a\n3. Action-Reaction: for every action (force), there is an equal and opposite reaction.", StackId = physicsStack.Id };
    var card9 = new Flashcard { Question = "Briefly explain the conclusions of Newton's shell theorem.", Answer = "For an object outside of a uniform sphere, all the gravitational force of that sphere can be counting as of coming from the center of it. For an object inside a spherically symmetric shell, there is no net gravitational force on it.", StackId = physicsStack.Id };
